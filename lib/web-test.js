...
class WebTest {
  ...
  constructor(config, reporter) {
    ...
    //console.log('WebTest start as', this);
  }
  ...
  runScenario() {
    ...
    return runPromiseInSequence(this.testCases)
      ...
      .catch(e => { // important to see error, do NOT remove it
        console.log(e.stack);
        console.log(this.reporter.print())
        throw e;
      });
  }
  ...
  registerExpression(string, namedFunc) {
    ...
    WebTest.expressions[string] = {
      regExp : new RegExp(`^${reStr}$`, 'i'),
      funcName: funcName,
      func: func
    };
    WebTest.prototype[funcName] = func;
  }
...
WebTest.getCommandFrom = (testStep) => {

  let expression, command;

  for (var key in WebTest.expressions) {
    let expr = WebTest.expressions[key];
