...
class Reporter {
  ...
  /**
   * set reporting items to be set to start/success/fail
   ...
   * @param testCaseName {string} optional, the name of testCase
   */
  success(level, testCaseName) {
    this.__setStatus(level, Status.SUCCESS, testCaseName);
  }
  ...
  fail(level, testCaseName) {
    this.__setStatus(level, Status.ERROR, testCaseName);
  }
