...
class WebTestDriver {
  ...
  showCircleOn(el) {
    let elLocation, elDimension;
    return Promise.resolve(el)
      .then(el => el.getLocation())
      .then(location => {
        elLocation = location;
        return el.getSize();
      }).then(dimension => {
  ...
  findBy(by, string, isVisible=true) {
    ...
    return this.driver.wait(
      ...
      .then(_ => this.showCircleOn(this.lastFoundElement))
      .then(_ => this.driver.sleep(this.config.speed))
      .then(_ => this.lastFoundElement)
      .catch(e => {throw e;});
  }
